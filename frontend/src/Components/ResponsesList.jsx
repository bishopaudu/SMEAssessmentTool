        ...
        <List>
          {groupedResponses.map((group, index) => (
            <Paper key={group.submissionId} elevation={3} style={{ marginBottom: '20px', padding: '10px' }}>
              ...
              <Typography variant="body2">Submitted on: {new Date(group.createdAt).toLocaleString()}</Typography>
              <Divider style={{ margin: '10px 0' }} />
              {group.answers.map((answer, idx) => (
                <Box key={answer._id} style={{ marginBottom: '10px' }}>
                  <Typography variant="subtitle1">
                    Question {idx + 1}: {answer.questionId?.question ? answer.questionId.question : 'Question not available'}
                  </Typography>
                  <Typography variant="body2">
                    Selected Option: {answer.selectedOption ? answer.selectedOption : 'No option selected'}
                  </Typography>
                  <Divider style={{ margin: '10px 0' }} />
                </Box>
              ))}
  ...
  useEffect(() => {
    const fetchResponses = async () => {
      ...
      try {
        ...
        groupResponsesBySubmission(response.data);
        console.log(response.data.selectedOption);
      } catch (error) {
        console.error('Error fetching responses:', error);
      } finally {
        setLoading(false); // Hide loading spinner
        ...
        <List>
          {groupedResponses.map((group, index) => (
            <Paper key={group.submissionId} elevation={3} style={{ marginBottom: '20px', padding: '10px' }}>
              <Typography variant="h6">Response Group {index + 1}</Typography>
              <Typography variant="body2">
                Submitted on: {new Date(group.createdAt).toLocaleString()} 
              </Typography>
              <Divider style={{ margin: '10px 0' }} />
              {group.answers.map((answer, idx) => (
                <Box key={answer._id} style={{ marginBottom: '10px' }}>
